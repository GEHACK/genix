...
- **Root partitions have a fixed size, not `100%`**: `100G` on the teammachine, `450G` on `cds` and `judgehost` (issue #66). The rest of the disk stays unallocated so imaging and capture only copy what is partitioned. disko fails on a disk smaller than the partition.
